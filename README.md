...
Registration and login are protected by Cloudflare Turnstile. `PUBLIC_TURNSTILE_SITE_KEY` is set in `[vars]` in `wrangler.toml`; `TURNSTILE_SECRET` is a worker secret (`wrangler secret put TURNSTILE_SECRET`).
