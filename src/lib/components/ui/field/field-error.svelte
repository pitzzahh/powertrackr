<script lang="ts">
  ...
  function isTypeError(message: string): boolean {
    return message.includes("must be a number") || message.includes("must be a valid number");
  }
  ...
  const hasContent = $derived.by(() => {
    ...
    if (isEmpty(value) && errors.some((e) => e.message && isTypeError(e.message))) {
      return false;
    }
  ...
  const singleErrorMessage = $derived(
    errors && errors.length === 1 && transformMessage(errors[0]?.message || "")
  );
</script>
...
{#if hasContent}
  ...
  >
    ...
    {:else if singleErrorMessage}
      {fieldName ? `${fieldName} ${singleErrorMessage}` : singleErrorMessage}
